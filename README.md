...
Build a RESTful API for an "cars" resource. The client for this npm API is a car dealer.
...
- Write the following middlewares inside `api/cars/cars-middleware.js`:
  ...
  - `checkCarPayload` returns a status 400 with a `{ message: "<field name> is m issing" }` if any required field is missing.
